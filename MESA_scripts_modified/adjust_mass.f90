      ...
      subroutine do_adjust_mass(s, species, ierr)
         ...
         if (s% deplete) then
            if (s% accrete_same_as_surface) then
                ...
                call get_xa_for_accretion(s, xaccrete, ierr)   ! xaccrete is defined now
                if (ierr /= 0) then
...
!!!
         ...
         call set_xa(s, nz, k_const_mass, species, xa_old, xaccrete, &
            rxm_old, rxm_new, mmax, old_cell_mass, new_cell_mass, new_xmstar, ierr)
         if (ierr /= 0) then
...
!!!!!!!!!!!!!!! Glenn edit:
! Change this routine such that it sets the outer layers of accreted material to the accreted composition.
      subroutine set_xa( &
            s, nz, k_const_mass, species, xa_old, xaccrete, &
            old_cell_xbdy, new_cell_xbdy, mmax, old_cell_mass, new_cell_mass, new_xmstar, ierr)
         ! set new values for s% xa(:,:)
         type (star_info), pointer :: s
         integer, intent(in) :: nz, k_const_mass, species
         real(dp), intent(in) :: mmax, new_xmstar
         real(dp), intent(in) :: xa_old(:, :), xaccrete(:)
         real(dp), dimension(:), intent(in) :: &
            old_cell_xbdy, new_cell_xbdy, old_cell_mass, new_cell_mass ! (nz)
         integer, intent(out) :: ierr
         integer :: k, j, op_err, k_accr_mass
         real(dp) :: frac
         real(dp) :: dt
         ...
         ! The goal is to set the accreted chemical composition
         ! even though there is also mass loss from stellar winds.
         ! So first, find the mass coordinate at the boundary of accreted mass.
         ! I have also added the variable s% accrete, which is the accretion rate in g/s.

         ! Kayla edit: dt is converted from seconds to years in the below 'frac' equation.

         k_accr_mass = 0
         if (s% deplete) then
            write(*,*) 'new_xmstar value:', new_xmstar
            write(*,*) 'dt value (direct from structure):', s% dt
            write(*,*) 'Accretion value (s% accrete, from adjust_mass.f90)', s% accrete

            dt = s% dt
            write(*,*) 'Timestep (dt) value:', dt, 'seconds', dt/31556952, 'years'  ! Kayla edit
            !frac = (new_xmstar - (s% accrete * s% dt))  / new_xmstar    ! mass fraction of accreted layer
            frac = (new_xmstar - (s% accrete * (dt/31556952)))  / new_xmstar    ! Kayla edit
            write(*,*) 'frac value: ', frac
         ...
         write(*,2) 'k_accr_mass: ', k_accr_mass

         ! These cells are in the accreted region and should get the accreted chemical composition.
         ! This does not feature mixing in a partially accreted cell,
         ! so care should be taken when the accretion rate is of the order of the mass in the outer cells.
         ! This is not a problem for my purposes, but could be when accreting asteroids on white dwarfs...
         if (k_accr_mass >= 1) then
            do k = 1, k_accr_mass
                do j=1,species
                    s% xa(j,k) = xaccrete(j)/sum(xaccrete(:))
                end do
            end do
         end if
         ! we want to set everything inside the accreted mass coordinate to remain the same.
!!!
...
!$OMP PARALLEL DO PRIVATE(k, op_err) SCHEDULE(dynamic,2)
         do k = k_accr_mass + 1, k_const_mass
            op_err = 0
